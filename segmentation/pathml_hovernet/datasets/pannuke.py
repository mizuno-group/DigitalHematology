...
from pathml_hovernet.datasets.base_data_module import BaseDataModule
from pathml_hovernet.datasets.utils import pannuke_multiclass_mask_to_nucleus_mask
from pathml_hovernet.ml.hovernet import compute_hv_map
from pathml_hovernet.utils import download_from_url
...
class PanNukeDataset(data.Dataset):
    ...
    def __init__(
        ...
        self.tissue_limit = tissue_limit
        ...
        if self.fold_ix is None:
            if self.tissue_limit == "tissue_name":
                paths = list(imdir.glob("*"))
            else:
                paths = list(imdir.glob(f"*_{self.tissue_limit}*"))
        else:
            if self.tissue_limit == "tissue_name":
                paths = list(imdir.glob(f"fold{fold_ix}*"))
            else:
                paths = list(imdir.glob(f"fold{fold_ix}*{self.tissue_limit}*"))
...
class PanNukeDataModule(BaseDataModule):
    ...
    def _get_dataset(self, fold_ix, augment=True):
        ...
        return PanNukeDataset(
            ...
            tissue_limit=self.tissue_limit,
        )
